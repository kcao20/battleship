...
def add_userC(username):
    ...
    c.execute(
        "SELECT * FROM usersClassic WHERE LOWER(username) = LOWER(?)", (username,)
    )
    check = c.fetchone()
    ...
    if check is not None:
        ...
        c.execute(
            """INSERT INTO usersClassic(username,gamesPlayed,gamesWon,gamesLost,shotsMissed,shotsLanded) VALUES(?, ?, ?, ?, ?, ?)""",
            (username, 0, 0, 0, 0, 0),
        )
    db.commit()
    db.close()
...
def add_userM(username):
    ...
    if check is not None:
        ...
        c.execute(
            """INSERT INTO usersModern(username,gamesPlayed,gamesWon,gamesLost,shotsMissed,shotsLanded,powersUsed) VALUES(?,?,?,?,?,?,?)""",
